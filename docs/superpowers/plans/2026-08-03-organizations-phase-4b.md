...
- Pass `orgKeys` to `updateCipher(authedFetch, session, cipher, draft, orgKeys)` (line ~614) and `createCipher(authedFetch, session, draft, orgKeys)` (createVaultItem path — note `createCipher`'s final signature is `(authedFetch, session, draft, orgKeys)`, no `extraPayload`). For `createVaultItem`, `orgKeys` is harmless (a personal draft resolves to the personal key).
- **Attachment functions (folded in from the Task 4 gap):** `getCipherKeys` has two more callers, `uploadCipherAttachment` (vault.ts:~270) and `downloadCipherAttachmentDecrypted` (vault.ts:~447), which Task 4 left unthreaded. In `webapp/src/lib/api/vault.ts`, ADD an `orgKeys: Record<string, Uint8Array> | undefined` param to BOTH function signatures and pass it to their internal `getCipherKeys(cipher, ..., orgKeys)` call. Then in `useVaultSendActions.ts`, pass `orgKeys` at ALL 4 call sites: `uploadCipherAttachment` at lines ~522 (import path — personal, pass `orgKeys` anyway; a null/personal cipher resolves to personal), ~560 (createVaultItem — personal), ~623 (updateVaultItem — MAY be an org cipher → real org-awareness needed), and `downloadCipherAttachmentDecrypted` at ~647 (MAY be an org cipher → real org-awareness needed). Fail-closed applies: uploading/downloading an attachment on an org cipher whose org key is unavailable throws `OrgKeyUnavailableError` (caught → `txt_org_key_unavailable` toast), never uses the personal key.
...
In `repairCipherKeyMismatches`: add an `orgKeys` param; remove the `if (cipher.organizationId) continue;` at line 1259; pass `orgKeys` to the `updateCipher(...)` call (line ~1262) and to `hasItemKeyFieldMismatch` (make it org-aware via `resolveCipherBaseKey`, skipping — treating as "no mismatch" / `continue` — when the org key is unavailable). **IMPORTANT (Task 4 arg-order note):** `updateCipher`'s signature is now `(authedFetch, session, cipher, draft, orgKeys, extraPayload?, options?)` — `orgKeys` is the 5th positional arg BEFORE `extraPayload`. The existing repair call passes `{ preserveRevisionDate: true }` as `extraPayload` and `{ webRepair: true }` as `options`; you must INSERT `orgKeys` as the 5th arg, i.e. `updateCipher(authedFetch, session, cipher, draftFromDecryptedCipher(cipher), orgKeys, { preserveRevisionDate: true }, { webRepair: true })` — do NOT leave `{ preserveRevisionDate: true }` binding to the `orgKeys` slot (that is the current tsc:1299 type error the amendment resolves).
